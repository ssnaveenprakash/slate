...
export class DatePickerComponent {
  ...
  readonly fieldId = `slt-date-picker-${++DatePickerComponent._idCounter}`;
  ...
  private _validate(input: HTMLInputElement): string {
    ...
    if (v.rangeUnderflow) return `Date must be on or after ${input.min}.`;
    if (v.rangeOverflow) return `Date must be on or before ${input.max}.`;
    return '';
  }
}
